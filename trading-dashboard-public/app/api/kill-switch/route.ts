...
export async function POST(req: Request) {
  try {
    ...
    if (typeof rawBody !== 'object' || rawBody === null || typeof (rawBody as Record<string, unknown>).active !== 'boolean') {
      return NextResponse.json({ ok: false, error: 'Invalid request body: active (boolean) required' }, { status: 400 });
    }
    const body = rawBody as { active: boolean; reason?: string; pin?: string };
    const now  = new Date().toISOString();
    const user = session.user?.email ?? session.user?.name ?? 'unknown';

    if (body.active) {
      const expectedPin = process.env.KILL_SWITCH_PIN;
      if (expectedPin && !timingSafeStringEqual(body.pin ?? '', expectedPin)) {
    ...
    if (process.env.NODE_ENV === 'development') console.error('[kill-switch] POST error:', err);
